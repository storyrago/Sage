...
구독만 지우면 그 방을 보고 있던 다른 탭은 아무 안내 없이 조용해진다. 회수한 **방마다 1건**을 `/user/queue/errors`로 보낸다. `destination`은 그 방의 채팅 목적지(`/sub/chatrooms/{id}`)다. 개인 목적지 전송은 그 회원의 모든 세션에 배달되므로 세션마다 따로 보내지 않는다 — 세션별로 보내면 같은 토스트가 세션 수만큼 중복된다.
